...
def get_prefix_and_measure(s):
    ...
    if len(s) == 2: 
        ...
        if not prefix in PREFIX_DICT:
            print('Problem!!  Unrecognized prefix')
        ...
        print('Problem.  What is this unit specifying??? %s' % s)
    return (prefix, measure)


def convert_unit(species, orig_unit, target_unit):
    '''
    orig_val is the VALUE of the original unit.  For example, if given 20 nMol/L,
    then orig_val = 20.  
    ...
    species is a string, so we use the correct conversion constant. Needs to be one of the 
    keys in the MOL_WT_MAP above.
    '''
    ...
    conversion_numerator_unit, conversion_denominator_unit = conversion_unit.split(PER)

	# handle numerators:    
    orig_numerator_prefix, orig_numerator_measure = get_prefix_and_measure(orig_numerator_unit)
    target_numerator_prefix, target_numerator_measure = get_prefix_and_measure(target_numerator_unit)
    conversion_numerator_prefix, conversion_numerator_measure = get_prefix_and_measure(conversion_numerator_unit)

	# handle denominators
    orig_denominator_prefix, orig_denominator_measure = get_prefix_and_measure(orig_denominator_unit)
    target_denominator_prefix, target_denominator_measure = get_prefix_and_measure(target_denominator_unit)
    conversion_denominator_prefix, conversion_denominator_measure = get_prefix_and_measure(conversion_denominator_unit)

    ##
    if (orig_numerator_measure == conversion_numerator_measure) \
        and \
        (target_numerator_measure == conversion_denominator_measure): # e.g. converting from ng to nM and conversion is given as x g/M
        numerator_factor = ((orig_numerator_prefix * conversion_denominator_prefix)/(target_numerator_prefix * conversion_numerator_prefix))*(1.0/conversion_factor)
    ...
    if (orig_numerator_measure == conversion_denominator_measure) \
        ...
        numerator_factor = ((orig_numerator_prefix * conversion_numerator_prefix)/(target_numerator_prefix * conversion_denominator_prefix))*conversion_factor
